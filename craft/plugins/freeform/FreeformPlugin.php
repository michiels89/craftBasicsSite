...
class FreeformPlugin extends BasePlugin
{
    ...
    const EVENT_BEFORE_VALIDATE = 'onBeforeValidate';
    const EVENT_AFTER_VALIDATE  = 'onAfterValidate';
    ...
    /** @var string */
    private $pluginDescription = 'The most intuitive and powerful form builder for Craft.';
    ...
    public function registerCpRoutes()
    {
        return [
            ...
            'freeform/forms/duplicate'                                  => ['action' => 'freeform/forms/duplicate'],
            ...
            'freeform/settings/(?P<template>[\w_\-]+)'                  => ['action' => 'freeform/settings/provideSetting'],
        ];
    }
    ...
    {
        $recaptcha = 'Solspace\Freeform\Library\Pro\Fields\RecaptchaField';
        if (class_exists($recaptcha)) {
            craft()->on('freeform_fields.onBeforeValidate', function (Event $event) use ($recaptcha) {
                /** @var AbstractField $field */
                $field = $event->params['field'];

                if ($field instanceof $recaptcha) {
                    $response = craft()->request->getPost('g-recaptcha-response');
                    if (!$response) {
                        $field->addError(Craft::t('Please verify that you are not a robot.'));
                    } else {
                        static $isValid;

                        if (null === $isValid) {
                            $secret = freeform()->settings->getSettingsModel()->recaptchaSecret;
                            $client = new Client();
                            ...
                            $request  = $client->post(
                                ...
                                [
                                    'secret'   => $secret,
                                    'response' => $response,
                                    'remoteip' => craft()->request->getIpAddress(),
                                ]
                            );
